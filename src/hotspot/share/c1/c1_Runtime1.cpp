...
JRT_ENTRY(void, Runtime1::deoptimize(JavaThread* current, jint trap_request))
  ...
  if (action == Deoptimization::Action_make_not_entrant) {
    if (nm->make_not_entrant(nmethod::ChangeReason::C1_deoptimize, true /* OK to recompile */)) {
...
JRT_ENTRY(void, Runtime1::patch_code(JavaThread* current, C1StubId stub_id ))
  ...
  if (deoptimize_for_volatile || deoptimize_for_atomic) {
    ...
    if (nm != nullptr) {
      nm->make_not_entrant(nmethod::ChangeReason::C1_codepatch, true /* OK to recompile */);
    }
...
void Runtime1::patch_code(JavaThread* current, C1StubId stub_id) {
  ...
  if (is_patching_needed(current, stub_id)) {
    ...
    nmethod* nm = CodeCache::find_nmethod(caller_frame.pc());
    if (nm != nullptr) {
      nm->make_not_entrant(nmethod::ChangeReason::C1_deoptimize_for_patching, true /* OK to recompile */);
    }
  }
...
JRT_ENTRY(void, Runtime1::predicate_failed_trap(JavaThread* current))
  ...
  nm->make_not_entrant(nmethod::ChangeReason::C1_predicate_failed_trap, true /* OK to recompile */);
