...
address Deoptimization::deoptimize_for_missing_exception_handler(nmethod* nm) {
  ...
  nm->make_not_entrant(nmethod::ChangeReason::missing_exception_handler, true /* OK to recompile */);
...
JRT_ENTRY(void, Deoptimization::uncommon_trap_inner(JavaThread* current, jint trap_request)) {
...

  {
    ...
    // Recompile
    if (make_not_entrant) {
      if (!nm->make_not_entrant(nmethod::ChangeReason::uncommon_trap, true /* OK to recompile */)) {
