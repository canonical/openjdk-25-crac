    ...
    if ((bci != InvocationEntryBci && !can_be_osr_compiled(mh, level))) {
      if (level == CompLevel_full_optimization && can_be_osr_compiled(mh, CompLevel_simple)) {
        ...
        if (osr_nm != nullptr && osr_nm->comp_level() > CompLevel_simple) {
          ...
          osr_nm->make_not_entrant(nmethod::ChangeReason::OSR_invalidation_for_compiling_with_C1, false /* recompiling by ourselves */);
        }
        compile(mh, bci, CompLevel_simple, THREAD);
      }
      return;
    ...
    if (mh() != imh()) { // If there is an enclosing method
      {
        ...
        if (max_osr_level == CompLevel_full_optimization) {
          ...
          if (make_not_entrant) {
            ...
            nm->make_not_entrant(nmethod::ChangeReason::OSR_invalidation_back_branch, false /* recompiling by ourselves */);
          }
        }
